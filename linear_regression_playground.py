...
def Cost2d(x0, ):
    x = x0[0:N]
    W = x0[(N):(N*M+N)].reshape([M,N])
    x.shape = [x.shape[0],1]
    y_.shape = [y_.shape[0],1]
    return ((y_ - W@x).transpose() @ C_n_inv @ (y_-W@x)) + (np.ndarray.flatten(W) - np.ndarray.flatten(Z)).transpose() @ C_A_inv @ (np.ndarray.flatten(W) - np.ndarray.flatten(Z))
...
opts = {'disp': False, 'maxiter': 70, 'gtol': 1.e-5, 'return_all': True}
#opts = {'disp': False}
...
opts = {'disp': False, 'maxiter': 45, 'gtol': 1.e-7, 'return_all': True}
